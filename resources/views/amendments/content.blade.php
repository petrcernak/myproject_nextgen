@extends('layouts.app')
@section('title', __('Edit amendment') . ' — ' . $amendment->code)
...
<div class="page-header">
    <h1 style="font-size:1.1rem">{{ __('Edit items') }}: {{ $amendment->code }} {{ $amendment->name }}</h1>
    <a href="{{ route('amendments.show', $amendment) }}" class="btn btn-secondary">{{ __('← Back to overview') }}</a>
</div>

{{-- Add new item form --}}
<div class="card card-body" style="margin-bottom:1.5rem;max-width:750px">
    <form method="POST" action="{{ route('amendments.items.store', $amendment) }}">
        @csrf
        ...
        <div class="form-group">
            ...
            <select name="contract_item_id" required>
                ...
                @foreach($amendment->contract->items as $ci)
        ...
        <table>
            ...
            <tbody>
                ...
                <tr style="background:#eff6ff">
                    <td colspan="3">
                        <form method="POST" action="{{ route('amendment-items.update', $item) }}" style="display:flex;gap:.75rem;align-items:flex-end;padding:.25rem 0">
                            @csrf @method('PUT')
                            ...
                            <a href="{{ route('amendments.content', $amendment) }}" class="btn btn-secondary btn-sm">{{ __('Cancel') }}</a>
                        </form>
                    </td>
                    <td></td>
                    <td></td>
                ...
                <tr>
                    ...
                    <td style="text-align:right">
                        <a href="{{ route('amendment-items.edit', $item) }}" class="btn btn-secondary btn-sm">{{ __('Edit') }}</a>
                        <form method="POST" action="{{ route('amendment-items.destroy', $item) }}" style="display:inline" onsubmit="return confirm('{{ __('Really delete?') }}')">
                            @csrf @method('DELETE')
