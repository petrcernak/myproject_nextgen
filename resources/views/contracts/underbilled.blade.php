...
<div class="page-header">
    <h1 style="font-size:1.1rem">{{ __('Underbilled contracts') }}
...
@if($contracts->isEmpty() && !request()->hasAny(['company_id','currency','date_from','date_to']))
    <div class="card"><div class="empty"><strong>{{ __('No underbilled contracts') }}</strong></div></div>
@else
...
<table class="ltbl" style="font-size:13px">
    <thead>
        <tr>
            ...
            <th style="min-width:130px">{{ __('Remaining') }}</th>
            <th style="min-width:70px;text-align:right">
                <button type="submit" class="btn btn-secondary btn-sm">{{ __('Filter') }}</button>
                @if(request()->hasAny(['company_id','currency','date_from','date_to']))<a href="{{ route('contracts.underbilled') }}" class="btn btn-secondary btn-sm">×</a>@endif
            </th>
        </tr>
    </thead>
    <tbody>
        ...
        <tr>
            ...
            <td style="text-align:right"><span style="font-size:11px;font-weight:600;color:{{ $pct >= 90 ? '#16a34a' : ($pct >= 50 ? '#ca8a04' : '#dc2626') }}">{{ $pct }} %</span></td>
            <td style="text-align:right;font-weight:600;color:#1d4ed8">{{ number_format(abs($contract->stat_diff), 2, ',', ' ') }}</td>
            <td></td>
        </tr>
        @empty
        <tr><td colspan="11" style="text-align:center;color:#9ca3af;padding:2rem 1rem">{{ __('No underbilled contracts match the filters.') }}</td></tr>
        @endforelse
    </tbody>
</table>
</div>
