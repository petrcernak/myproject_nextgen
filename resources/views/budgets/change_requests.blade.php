@extends('layouts.app')
@section('title', __('Change Requests').' — '.$budget->name)
...
<div class="page-header">
    <h1>{{ __('Change Requests') }} <span style="font-size:.7em;font-weight:400;color:#6b7280">{{ $budget->name }}</span></h1>
    <div style="font-size:12px;color:#6b7280">
        {{ __('Read-only — aggregated from open and closed change requests on linked contracts.') }}
    </div>
</div>
    ...
    <table style="font-size:13px">
        <thead>
            <tr>
                ...
                <th style="text-align:right;width:140px">{{ __('Change Requests') }}</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align:right;font-weight:{{ $antCr != 0 ? '600' : '400' }};color:{{ $cc($antCr) }}">
                    {{ $antCr != 0 ? $sgn($antCr) : '—' }}
                </td>
            </tr>
            @endforeach
