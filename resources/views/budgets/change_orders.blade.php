@extends('layouts.app')
@section('title', __('Change Orders').' — '.$budget->name)
...
<div class="page-header">
    <h1>{{ __('Change Orders') }} <span style="font-size:.7em;font-weight:400;color:#6b7280">{{ $budget->name }}</span></h1>
    <div style="font-size:12px;color:#6b7280">
        {{ __('Read-only — change order amounts from linked contracts, aggregated per budget item.') }}
    </div>
</div>
    ...
    <table style="font-size:13px">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align:right;font-weight:{{ $co != 0 ? '600' : '400' }};color:{{ $cc($co) }}">
                    {{ $co != 0 ? $sgn($co) : '—' }}
                </td>
            </tr>
            @endforeach
