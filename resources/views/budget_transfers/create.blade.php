@extends('layouts.app')
@section('title', __('New transfer').' — '.$budget->name)
...
<form method="POST" action="{{ route('budgets.transfers.store', $budget) }}">
    @csrf
        ...
        <div class="form-row" style="align-items:flex-end;gap:1rem">
                ...
                <select name="from_budget_item_id" required style="width:100%">
                    ...
                    @foreach($budget->categories->whereNull('parent_id') as $cat)
                        <optgroup label="{{ $cat->code ? $cat->code.' — ' : '' }}{{ $cat->name }}">
                            @include('budget_transfers._item_options', ['category' => $cat, 'selected' => old('from_budget_item_id')])
                        </optgroup>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group" style="flex:1">
                ...
                <select name="to_budget_item_id" required style="width:100%">
                    ...
                    @foreach($budget->categories->whereNull('parent_id') as $cat)
                        <optgroup label="{{ $cat->code ? $cat->code.' — ' : '' }}{{ $cat->name }}">
                            @include('budget_transfers._item_options', ['category' => $cat, 'selected' => old('to_budget_item_id')])
                        </optgroup>
                    @endforeach
                </select>
            </div>
    ...
    <div class="form-actions">
        <button type="submit" class="btn btn-primary">{{ __('Save transfer') }}</button>
